...
public class Tester {
    ...
    static int PUSH_COUNT = 1000000;
    //发送消息的线程数
    static int PUSH_THREAD_COUNT = 4;
    //发送线程往n个topic发消息
    static int PUSH_TOPIC_COUNT = 4;
    //消费消息的线程数
                ...
                while (true) {
                    ...
                    if (msg == null) {
                        ...
                        if (j != posTable.get(mapkey)) {
                            System.out.println(String.format("body error topic %s number:%d", topic, j));
                            System.exit(0);
                        }
                        if (!msg.headers().getString(MessageHeader.SEARCH_KEY).equals("hello")) {
                            System.out.println(String.format("header error topic %s number:%d", topic, j));
                            System.exit(0);
                        }
    ...
    public static void main(String args[]) {
        try {
            ...
            for (int i = 0; i < PUSH_THREAD_COUNT; i++) {
                //Continuous and randomly select topic
            ...
            System.out.println(String.format("push ove time cost %d push count %d", time2 - time1, pushCount.get()));
            System.out.println("start tp pull");
            ...
            System.out.println(String.format("pull over time cost %d pull count %d", time3 - time2, pullCount.get()));
